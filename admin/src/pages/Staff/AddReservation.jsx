...
const AddReservationStaff = () => {
  ...
  const calculateHours = () => {
    ...
    // Prevent negative hours or same time selection
      ...
      <form onSubmit={handleSubmit} className='m-5 w-full'>
        ...
        <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll text-gray-700'>
            ...
            <div className='mb-4'>
              ...
              <select
                ...
                {selectedFacility.pricingPackages.map((pkg, idx) => (
                  <option key={idx} value={pkg.name}>{pkg.name} — {pkg.description} ({formatPrice(pkg.feePerHour)}/hr)</option>
                ))}
              </select>
            </div>
          )}
          ...
          {startTime && endTime && totalHours <= 0 && (
            <p className="text-red-500 text-sm mt-1 mb-4">End time must be after start time</p>
          )}
          ...
          {selectedPackage && startTime && endTime && totalHours > 0 && (
            <div className='mb-4 p-3 bg-gray-50 rounded'>
              ...
              <p className='font-medium'>Total Price: <span className='text-primary'>{formatPrice(totalPrice)}</span></p>
            </div>
          )}
          ...
          <div className='mb-4 p-3 bg-blue-50 rounded'>
            <p className='text-sm text-gray-600'>
              <strong>Handled by:</strong> {user?.fullName || user?.name || user?.username || 'Staff User'}
            </p>
          ...
          <div className='bg-white rounded-2xl shadow-xl p-6 max-w-md w-full mx-4'>
            ...
            <div className='bg-gray-50 p-4 rounded-lg mb-4'>
              ...
              <div className='space-y-2 text-sm'>
                ...
                <p><strong>Handled by:</strong> {user?.fullName || user?.name || user?.username || 'Staff User'}</p>
...
export default AddReservationStaff